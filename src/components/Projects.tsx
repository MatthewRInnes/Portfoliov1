...
const projects = [
	...
	},
  {
    ...
    video: "/assets/videos/Digital Agency Portfolio.mp4",
    liveLink: 'https://example.com',
...
const Projects = () => {
  ...
  // Intersection observers for project card animations
  const project1Ref = useIntersectionObserver({ direction: 'left', threshold: 0.1 });
  const project2Ref = useIntersectionObserver({ direction: 'right', threshold: 0.1 });
  const project3Ref = useIntersectionObserver({ direction: 'left', threshold: 0.1 });
  const project4Ref = useIntersectionObserver({ direction: 'right', threshold: 0.1 });

  // Map of project references for animation control
  const projectRefs = {
    1: project1Ref,
    2: project2Ref,
    3: project3Ref,
  ...
  return (
    ...
    <section id="projects" className="py-24 bg-background dark:bg-navy">
      <div className="container mx-auto px-6">
        ...
        <div className="flex flex-wrap gap-4 mb-12">
          {categories.map((category) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium tracking-wide transition-colors duration-300 ${
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {filteredProjects.map((project) => {
            const cardRef = projectRefs[project.id];
            ...
            return (
              ...
              >
                ...
                <div className="p-6">
                  {/* Project title */}
                  <h3 className="text-xl font-semibold text-foreground dark:text-lightestSlate mb-2 tracking-tight">
                    {project.title}
                  </h3>
                  
                  {/* Project description */}
                  <p className="text-foreground/70 dark:text-lightSlate mb-4 leading-relaxed tracking-wide">
                    {project.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-3 py-1 bg-secondary dark:bg-navy/60 text-foreground dark:text-lightSlate rounded-full text-sm font-medium tracking-wide"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="flex gap-4 items-center">
                    ...
                    <Link
                      ...
                      className="ml-auto px-4 py-2 bg-teal text-white rounded-md hover:bg-teal/90 transition-colors duration-300 font-medium tracking-wide"
                    >
                      View More
                    </Link>
                  </div>
