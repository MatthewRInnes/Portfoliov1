    ...
    <nav className="w-full bg-background/80 backdrop-blur-md fixed top-0 left-0 z-50 border-b border-border dark:border-slate/20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-28">
          ...
          <ul className="flex space-x-8">
            {navItems.map((item, index) => (
              <li key={index}>
                <a 
                  href={item.href}
                  className="text-foreground dark:text-lightSlate hover:text-gradient-teal dark:hover:text-gradient-teal transition-colors duration-700 relative group flex items-center font-medium text-base tracking-wide"
                >
        ...
        <div className="flex items-center gap-4 ml-auto">
          ...
          <button 
            onClick={toggleTheme}
            className="p-3 rounded-full bg-secondary hover:bg-accent/20 dark:bg-navy/80 dark:hover:bg-teal/20 transition-colors duration-300 shadow-md font-medium"
          ...
          <Link 
            to="/" 
            className="flex items-center gap-2 text-foreground dark:text-lightestSlate relative group mt-2 lg:mt-0 font-semibold"
            ...
            <ul className="space-y-8 text-center">
              {navItems.map((item, index) => (
                <li key={index}>
                  <a
                    href={item.href}
                    className="text-foreground dark:text-lightSlate hover:text-gradient-teal dark:hover:text-gradient-teal text-2xl transition-colors duration-700 relative group flex items-center justify-center font-medium tracking-wide"
                    onClick={toggleMenu}
                  >
